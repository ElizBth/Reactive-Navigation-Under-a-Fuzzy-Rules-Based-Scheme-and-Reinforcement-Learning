...
class SarsaNavigationApproach(Sarsa):
    version = 1
    trial = 0
...
if __name__ == "__main__":
    ...
    print(len(_states))
    robotBatteryFQLSimulation = SarsaNavigationApproach(_target, _charging_station, obstacles_total[0], _max_success)
    delay = np.zeros(15)
    for index in range(1):
        ...
        if SAVE_LOG:
            np.savetxt("/home/lizzy/PycharmProjects/RobotPathPlanningBasedOnBatteryBehaviour/simulations/log_peer_sarsa/log_sarsa_peer_test_1_" + str(index) + ".csv",
               robotBatteryFQLSimulation.log,
               delimiter=", ",
               fmt='% s')

    if SAVE_TIME:
        np.savetxt("/home/lizzy/PycharmProjects/RobotPathPlanningBasedOnBatteryBehaviour/simulations/log_peer_sarsa/log_sarsa_peer_test_1_delay.txt",
                   delay,
                   delimiter=", ",
                   fmt='% s')
