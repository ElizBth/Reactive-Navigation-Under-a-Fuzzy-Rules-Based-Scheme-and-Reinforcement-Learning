...
class Sarsa:
    ...
    def select_action(self, trial, state):
    ...
    def step_execution(self, state, trial, action):
        new_state = self.task_execution(action)

        new_action = self.select_action(new_state, trial)

        reward = self.get_reward(action, new_state)
        best_q = self.q_table[new_state, new_action]
        ...
        return new_state, new_action, reward
